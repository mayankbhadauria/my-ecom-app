...
constructor(private http: HttpClient){
  ...
  public getProducts():Observer<any>{

    //return this.http.get<any>('/api/events').pipe(catchError(this.handleError<any>('getEvent', [])));

    const subject = new Subject;
    setTimeout(() => {
      subject.next(this.PRODUCTS); subject.complete();
    }, 100);
    return subject;
  }
